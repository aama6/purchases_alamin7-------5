...
export const ExcludedOffersSection: React.FC = () => {
  ...
  const generateAutoReasons = (vendor: string) => {
    ...
    // ✅ مقارنة الأسعار مع التكلفة التقديرية لكل صنف
        ...
        if (lineItem && poItem.estimatedCost) {
          ...
          if (estimatedCost > 0 && offeredPrice > 0) {
            ...
            if (Math.abs(difference) > 0.01) { // تجاهل الفروقات الطفيفة
              if (difference > 0) {
                reasons.specReasons.push(`الصنف ${poItem.lineNumber}: السعر أعلى من التكلفة التقديرية بنسبة ${percentage}%`);
              } else {
                reasons.specReasons.push(`الصنف ${poItem.lineNumber}: السعر أقل من التكلفة التقديرية بنسبة ${Math.abs(Number(percentage))}%`);
              }
            }
          }
        }
    ...
    // ✅ مقارنة المواصفات المطلوبة مع المواصفات المقدمة
    if (priceOffer.lineItems && poItems) {
      poItems.forEach(poItem => {
        const lineItem = priceOffer.lineItems?.find(li => li.lineNumber === poItem.lineNumber);
        ...
        if (poItem.specifications && lineItem?.vendorSpecifications) {
          ...
          if (missingSpecs.length > 0) {
            reasons.specReasons.push(`الصنف ${poItem.lineNumber}: مواصفات مطلوبة غير مقدمة (${missingSpecs.join(', ')})`);
          } else {
            ...
            if (!specsMatch) {
              reasons.specReasons.push(`الصنف ${poItem.lineNumber}: مواصفات غير مطابقة - ${mismatchedSpecs.join(', ')}`);
            } else {
              reasons.specReasons.push(`الصنف ${poItem.lineNumber}: مواصفات مطابقة`);
            }
          }
        } else if (poItem.specifications && (!lineItem?.vendorSpecifications || Object.keys(lineItem.vendorSpecifications || {}).length === 0)) {
          reasons.specReasons.push(`الصنف ${poItem.lineNumber}: لم يتم تقديم مواصفات`);
        }
      });
    }
...
// السعر
// يتم المقارنة بين سعر التكلفة التقديرية للوحدة وسعر الوحدة المقدمة من فبل الموردين على مستوى كل صنف وكتابه الزيادة أو النقص عن سعر التكلفة

// المواصفات
// يتم مقارنه بين المواصفات المطلوبة والمواصفات المقدمة 
// فاذا كانت المواصفات المقدمة نفس المواصفات المطلوبة يتم كتابه "مطابق"
//  واذا كانت المواصفات المقدمة تختلف عن المواصفات المطلوبة يتم كتابه غير مطابقه حيث ان المطلوب (..........كذا) والمقدم (..............كذا
// 2-
...
// 6-
// في صفحة المقارنة وبالتحديد في ملاحظات الاختلافات في المواصفات:
// أ- 

// اذا كانت المواصفات المقدمة نفس المواصفات المطلوبة يتم ذكر الصنف ويكتب مطابق للمواصفات المطلوبة
// ب- 
// اذا كان هناك اختلاف بين المواصفات فيجب ان يظهر اسم الصنف ويكتب هناك "غير مطابق" هناك اختلاف بين المواصفات حيث أن المطلوب (كذا...........) والمقدم (كذا..............)
// ج-
...
// وكذلك ملاحظات النقص في سعر الوحدة من الكلفة التقديرية
// ----------------------------------------
