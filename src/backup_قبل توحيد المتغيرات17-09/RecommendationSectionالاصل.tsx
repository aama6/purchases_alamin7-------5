...
// مكون جدول التوصية مع عرض الالتزامات المحسن وإدارة الرسائل
...
import { RecommendedOffer } from '../types';
...
export const RecommendationSection: React.FC = () => {
  ...
  // const getDynamicSignatures = (totalAmount: number, isPreliminary: boolean) => {
  //   if (isPreliminary) {
  //     // عند الطباعة الأولية
  //     if (totalAmount <= 150000) {
  //       // إذا كان المبلغ أقل من أو يساوي 150,000 ريال يمني
  //       return [
  ...
  //       ];
  //     } else {
  //       // إذا كان المبلغ أكبر من 150,000 ريال يمني
  //       return [
  //         { title: "المختص", name: "الاسم", role: "---------------------" },
  //         { title: "رئيس القسم", name: "الاسم", role: "---------------------" },
  //         { title: "مدير الإدارة", name: "الاسم", role: "---------------------" },
  ...
  // const getRecipient = (totalAmount: number) => {
  //   return totalAmount <= 150000 
  //     ? "الأخ/مدير عام المشتريات والمخازن" 
  //     : "الأخ/المدير العام التنفيذي";
  // };

  // إضافة دالة لحساب أكبر عرض بالريال اليمني
const getMaxOfferInYR = () => {
  return Math.max(...priceOffers.map(offer => offer.totalInYR || 0), 0);
};

// تعديل دالة التوقيعات الديناميكية
const getDynamicSignatures = (isPreliminary: boolean) => {
  const amount = isPreliminary 
    ? getMaxOfferInYR() // للطباعة الأولية: أكبر عرض
    : calculateTotalAwardedInYR(); // للطباعة النهائية: إجمالي الترسية
...
// تعديل دالة تحديد المخاطب بناءً على إجمالي الترسية
const getRecipient = () => {
  const totalAwarded = calculateTotalAwardedInYR(); // إجمالي الترسية بالريال
  return totalAwarded <= 150000 
    ? "الأخ/مدير عام المشتريات والمخازن" 
    : "الأخ/المدير العام التنفيذي";
};

// تعديل دالة تحديد المعتمد بناءً على إجمالي الترسية
const getApprover = () => {
  const totalAwarded = calculateTotalAwardedInYR(); // إجمالي الترسية بالريال
  return totalAwarded <= 150000 
    ? "مدير عام المشتريات والمخازن" 
    : "المدير العام التنفيذي";
};

// تعديل دالة تحديد نص التوقيع بناءً على إجمالي الترسية
...
// تعديل قسم التوقيعات في JSX
// const signatures = getDynamicSignatures(printType === 'preliminary');

// تعديل النص الثابت بعد التوقيعات
{printType === 'final' && (
  <div className="hidden print:block border border-gray-300 rounded-lg p-2 mt-4 print:mt-2">
    ...
    <div className="mt-2 font-bold text-left print:mt-0">
      <p className="font-medium">{getApprover()}</p>
  ...
  const handleAdditionalPrinting = () => {
    ...
    const financialLetterContent = `
      <div style="font-family: Arial, sans-serif; direction: rtl; text-align: right; padding: 6px; line-height: 1.2;">
        ...
        <p style="margin: 5px 0;">بناءً على موافقة ${getRecipient(calculateTotalAwardedInYR())} على الشراء بموجب الأوليات المرفقة، يرجى التوجيه باستكمال إجراءات سداد مستحقات الموردين:</p>
    ...
    <div className="bg-white rounded-lg shadow-lg p-4 print:shadow-none print:p-2 border border-gray-200 print-container">
                              ...
                              <div className="mt-3">
                                ...
                                <div className="bg-gray-100 p-2 rounded border text-sm max-h-40 overflow-y-auto">
                                  <table className="w-full text-sm border-collapse border border-gray-300">
                                    ...
                                    <tbody>
                                      {selectedOffer.lineItems.map((item: any, idx: number) => {
                          ...
                          {offer.lineItems && offer.lineItems.length > 0 && (
                            <div className="mt-2 overflow-x-auto">
                              <table className="w-full border border-gray-300 text-sm">
                                ...
                                <tbody>
                                  {offer.lineItems.map((li: any, idx: number) => {
      ...
      <div className="hidden print:block mt-2 space-y-2 print:mt-0 print:space-y-0">
        <div className="border-t border-gray-300 pt-2 print:my-0">
          <div className={`grid gap-0 text-center grid-cols-${signatures.length}`}>
        ...
        {printType === 'final' && (
          <div className="hidden print:block border border-gray-300 rounded-lg p-2 mt-4 print:mt-2">
            ...
            <p className="text-center">وشكراً...</p>
            <div className="mt-2 font-bold text-left print:mt-0">
              <p className="font-medium">{calculateTotalAwardedInYR() > 150000 ? "المدير العام التنفيذي" : "مدير عام المشتريات والمخازن"}</p>
              <p className="text-sm mt-2 print:mt-0">
...
// المنفذ
// 1-إصلاح مشكلة الطباعة النهائية
// 2. تعديل التوقيعات لتظهر بشكل أفقي وديناميكي

// 3. إظهار الالتزامات في جدول تفاصيل الأصناف المرسى عليها
// المطلوب إظهار الالتزامات التي يتم إدخالها في جدول عروض الأسعار على مستوى السطر في جدول تفاصيل الأصناف المرسى عليها.

// 4. إصلاح ظهور حقل المبلغ كتابة عند الترسية على إجمالي العرض
// المطلوب إصلاح ظهور حقل المبلغ كتابة عندما يتم الترسية على إجمالي العرض بدون إدخال أسعار الأصناف.

// 5. تقديم مقترحات تحسين وإضافة تعليقات توضيحية
// =================================================
// في ملف التوصية
